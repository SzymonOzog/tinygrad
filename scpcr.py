...
if __name__ == "__main__":
  ...
  for num, ast in (pbar:=tqdm(enumerate(ast_strs))):
    ...
    if ratio > 1.2 or single != -1 or True:
      slow.append((num, ratio, min(tm_cuda), to_function_name(lin.name)))
  ...
  print(f"final ratio:{average_tm_ptx/average_tm_cuda:5.2f}")
  for i1 in ii1:
    if i1 not in ii2:
      print("unused", i1)
  for i2 in ii2:
  ...
  for x in slow[-30:]:
    print(f"num {x[0]}, ratio:{x[1]}, time: {x[2]*1e6:7.2f} us, name: {x[3]}")
